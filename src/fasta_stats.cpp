...
*/
FastaStats::FastaStats(unsigned int ns, unsigned int nb, unsigned int mnl,
                       unsigned int mxl)
  : num_seqs(ns)
  , num_bases(nb)
  , min_len(mnl)
...
*/
bool
operator==(const FastaStats& stats1, const FastaStats& stats2)
{
  return stats1.num_seqs == stats2.num_seqs &&
         stats1.num_bases == stats2.num_bases &&
         stats1.min_len == stats2.min_len && stats1.max_len == stats2.max_len;
}
...
*/
ostream&
operator<<(ostream& out, const FastaStats& stats)
{
  out << "FastaStats(" << stats.num_seqs << ", " << stats.num_bases << ", "
...
*/
string
FastaStats::pretty(string filename)
{
  ...
  if (num_seqs > 0) {
    ...
    result << filename << "\t" << num_seqs << "\t" << num_bases << "\t"
...
*/
void
FastaStats::from_file(SeqFileIn& seq_file_in, unsigned minlen_threshold)
{
  unsigned this_len;
  CharString id;
  CharString seq;
